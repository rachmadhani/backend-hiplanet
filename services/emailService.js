...
`;
  ...
  async sendBuildUpdateEmail(toEmail, toName, buildVersion, patchNotes, build_platform = 'Windows') {
    const isWindows = String(build_platform).toLowerCase() === 'windows';
    const platformDisplay = isWindows ? 'Windows' : 'macOS';
    const downloadUrl = isWindows ? process.env.DOWNLOAD_URL_WINDOWS : process.env.DOWNLOAD_URL_MAC;
    const requirements = isWindows ? 'Windows 10+' : 'macOS 12+';

    const subject = `New Closed Beta Build Live (${platformDisplay}): Version ${buildVersion} - ${process.env.GAME_NAME || 'Hi Planet'}`;

    // Plain text version (fallback)
    const textBody =
      `Hi ${toName},

A new closed beta build (Version ${buildVersion}) is now live for ${process.env.GAME_NAME || 'Hi Planet'} on ${platformDisplay}!
...
==============================
- No public videos/screenshots until the ${process.env.LAUNCH_DATE || ''} launch
- Send feedback by replying to this email with any bugs, thoughts, or suggestions.

Thanks for testing ${process.env.GAME_NAME || 'Hi Planet'}.
...
<div style="font-family: 'Space Grotesk', -apple-system, BlinkMacSystemFont, Arial, sans-serif; line-height: 1.6; color: #1e293b; max-width: 600px; margin: 0 auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
  ...
  <p style="font-size: 15px; margin-bottom: 20px;">A new closed beta build (<strong>Version ${buildVersion}</strong>) is now live for <strong>${process.env.GAME_NAME || 'Hi Planet'}</strong> on <strong>${platformDisplay}</strong>. Thank you for your continued testing and support!</p>
  
  <div style="margin: 30px 0; padding: 20px; background-color: #f8fafc; border-left: 4px solid #ff6b35; border-radius: 0 8px 8px 0;">
    <h3 style="margin-top: 0; color: #0f172a; font-size: 14px; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 5px;">Download the Update</h3>
    <p style="font-size: 13px; color: #64748b; margin-bottom: 15px;">(${requirements})</p>
    <a href="${downloadUrl || '#'}" style="display: inline-block; padding: 12px 24px; background-color: #ff6b35; color: #ffffff; text-decoration: none; font-weight: bold; font-size: 14px; border-radius: 6px; box-shadow: 0 4px 6px -1px rgba(255, 107, 53, 0.2);">▶ Download Update for ${platformDisplay}</a>
  </div>
  
  <div style="margin-bottom: 25px;">
    <h3 style="color: #0f172a; font-size: 14px; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; margin-bottom: 12px; margin-top: 0;">What's New in this Build</h3>
...
${patchNotes}
  ...
  <div style="margin-bottom: 30px;">
    <h3 style="color: #0f172a; font-size: 14px; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; margin-bottom: 12px; margin-top: 0;">Reminders</h3>
    <ul style="padding-left: 20px; font-size: 14px; margin: 0; list-style-type: square;">
      <li style="margin-bottom: 6px;">Confidentiality: No public videos, screenshots, or streams until launch.</li>
      <li style="margin-bottom: 6px;">Feedback: Simply reply directly to this email with any bugs, thoughts, or suggestions!</li>
    </ul>
  </div>
...
`;
    ...
    try {
      ...
      console.log(`✓ Build update email sent successfully to ${toEmail}`);
      return true;
    } catch (err) {
      console.error('Mail send failed:', err.message);
      return false;
